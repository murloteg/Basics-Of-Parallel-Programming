...
#pragma omp parallel
    {
        while (true) {
            #pragma omp single
            {
                if (CheckStopCondition(upperPartSecondNorm, lowerPartSecondNorm)) {
                    exitFlag = true;
                }
            }
            if (exitFlag) {
            ...
            #pragma omp single
            {
                upperPartSecondNorm = 0;
            }

            #pragma omp for reduction(+: upperPartSecondNorm)
            for (int i = 0; i < size; ++i) {
    ...
    CleanUp(matrix, vectorOfSolution, vectorOfRightSide, resultOfMultiplicationByConst, resultOfMultiplication,
            resultOfSubtraction);
